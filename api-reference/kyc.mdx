---
title: "Individuals - KYC"
---

In order to use the Byzantine Prime application, an individual user needs to go through a KYC process. This is a regulatory obligation for individuals.

### 1. Accept the Terms

* **Accept the Byzantine Terms and Conditions**

The user must accept the [Byzantine Terms and Conditions](https://www.byzantine.fi/terms-and-conditions) before creating the user account. The `timestamp` of the acceptance is required in the `/submit/create-user` endpoint.

* **Accept the Bridge Terms of Service**

The following endpoint should be used to obtain a Terms of Service acceptance link. Integrators should then display this link to the end user, either via a redirect or an iFrame. End users must click it to accept the Bridge Terms of Service.

<Card title="Get a Bridge ToS link" icon="arrow-right" iconType="regular" horizontal href="/api-reference/terms-of-service/post-v1queryget-tos-acceptance-link">
  
</Card>

<Warning>
  The user must accept the Bridge Terms of Service before creating the user account. Note that the returned ToS agreement ID will be used in the next step to create the user account.
</Warning>

<Info>
  Please refer to [Account Creation](/documentation/account-management/account-creation) for more details about the Bridge ToS.
</Info>

### 2. Create the user account

The creation of the user account is done by calling this API endpoint:

<Card title="Create a user account" icon="arrow-right" horizontal href="/api-reference/user-accounts/create-a-byzantine-user">
  
</Card>

The following information needs to be provided:

- personal data about the user
- ID documents (passport or national ID card)
- proof of address
- optional - it is possible to add a list of `authenticators` to register user's passkey to their device

The individual user will receive an `account_id` and a `user_id`.

<Info>
  Please refer to the related endpoint page for more details about the requirements.
</Info>

### 3. KYC verification

Byzantine and Bridge will verify the individual's identity and address. Both Bridge and Byzantine may ask for additional details during this process.

Once completed, the user will receive an email notifying them of their account opening.

<Info>
  The KYC process usually takes only a few minutes.
</Info>