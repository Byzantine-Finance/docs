...
<Tabs>
  ...
  <Tab title="Passkey Creation">
    **Setup:**
    ...
    const credential = await client.webauthn.createPasskey(passkeyConfig);
    ```
    ...
    <Note>
      For non-discoverable credentials, you must store the credential ID returned from `createPasskey`. When authenticating transactions, pass this credential ID as the second parameter to `client.webauthn.getStamp(payload, credentialId)` (cf. **Passkey Authentication** tab).
    </Note>
