...
<Tabs>
  ...
  <Tab title="Passkey Creation">
    **Setup:**
    ...
    const { credentialIdBase64, attestationData, challengeFromClientData } =
    ...
    <Note>
      For non-discoverable credentials, you must store the credential ID returned from `createPasskey`, namely, `credentialIdBase64`. When authenticating transactions, pass this credential ID as the second parameter to `client.webauthn.getStamp(payload, credentialId)` (cf. **Passkey Authentication** tab).
    </Note>
