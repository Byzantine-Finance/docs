...
Each delivery body is JSON and shares a common envelope ([`WebhookLifecycleEventPayload`](#endpoint-reference)). The identifier fields that are populated (`accountId`, `userId`, `entityId`, `beneficiaryId`, `transactionId`) and the exact `data` object depend on the event type.
