...
Byzantine emits two families of lifecycle events, documented on their own pages:

- [Customers](/api-reference/webhooks/customers) - KYC/KYB onboarding status changes;
- [Transactions](/api-reference/webhooks/transactions) - deposit and withdrawal status changes.

`webhook.test` is used for test deliveries and is not directly subscribable.

## Delivery payload

Each delivery body is JSON and shares a common envelope ([`WebhookLifecycleEventPayload`](#endpoint-reference)). The event name is in `type`, the ids the event concerns are grouped under `related`, and the shape of `data` depends on the event type.
...
`related` always carries all five keys. Only the ones relevant to the event are set; the rest are `null` rather than absent, so you can read them without existence checks.

The same envelope is what [`GET /v1/query/events`](/api-reference/events/lists-persisted-lifecycle-events-owned-by-the-authenticated-integrator-newest-first) returns for each stored event, with a `createdAt` added. A receiver written against webhook deliveries can therefore parse replayed events unchanged.
...
- [`POST /v1/webhooks/subscriptions`](/api-reference/webhooks/create-a-webhook-subscription)
- [`GET /v1/webhooks/subscriptions`](/api-reference/webhooks/list-webhook-subscriptions)
- [`PATCH /v1/webhooks/subscriptions/{subscription_id}`](/api-reference/webhooks/update-a-webhook-subscription)
- [`DELETE /v1/webhooks/subscriptions/{subscription_id}`](/api-reference/webhooks/delete-a-webhook-subscription)
- [`POST /v1/webhooks/subscriptions/{subscription_id}/test`](/api-reference/webhooks/send-a-signed-test-webhook)
- [`GET /v1/webhooks/deliveries`](/api-reference/webhooks/list-webhook-delivery-history)
- [`POST /v1/webhooks/deliveries/{delivery_id}/retry`](/api-reference/webhooks/retry-a-webhook-delivery)
