...
<Info>
  A `user_id` is tied to a specific email and a specific `integrator_id`. Each email address can only be used once per integrator — attempting to create a second user with the same email will return an error. Refert to [Multi-accounts](/api-reference/multi-accounts) for more details.
</Info>
